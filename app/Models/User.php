...
class User extends Authenticatable
{
    ...
    /**
     * M-007: back-compat wrapper around the new CommissionCalculator
     * service.  Old code that still calls User::calculateCommission()
     * will keep working; new code should use
     * `app(\App\Services\CommissionCalculator::class)->compute(...)`
     * directly so the calculator can be injected / swapped in tests.
     *
     * Old behaviour preserved:
     *  - "no tiers defined" returns 0 (the caller used to see this as
     *    a silent sentinel); use {@see CommissionCalculator::compute()}
     *    directly if you need the exception instead.
     */
    public static function calculateCommission(
        ...
        try {
            return app(\App\Services\CommissionCalculator::class)->compute(
                $user,
                (int) $ticketTypeId,
                (int) $quantity,
                (int) $ticketOrderId,
        ...
        } catch (\App\Services\NoCommissionTierException $e) {
