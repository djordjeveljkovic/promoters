        ...
        <x-ds.card :title="__('How to sell a ticket')">
            <x-slot:body>
                <ol class="space-y-3 text-sm text-[color:var(--ds-text)]">
                    <li class="flex gap-3">
                        <span class="flex-shrink-0 w-6 h-6 rounded-full bg-indigo-100 text-indigo-700 text-xs font-semibold flex items-center justify-center">1</span>
                        <span>{{ __('Enter the customer email in the "Email" field.') }}</span>
                    </li>
                    <li class="flex gap-3">
                        <span class="flex-shrink-0 w-6 h-6 rounded-full bg-indigo-100 text-indigo-700 text-xs font-semibold flex items-center justify-center">2</span>
                        <span>{{ __('Pick a ticket type, choose quantity, and click "Add item".') }}</span>
                    </li>
                    <li class="flex gap-3">
                        <span class="flex-shrink-0 w-6 h-6 rounded-full bg-indigo-100 text-indigo-700 text-xs font-semibold flex items-center justify-center">3</span>
                        <span>{{ __('You can add up to 10 tickets in a single order.') }}</span>
                    </li>
                    <li class="flex gap-3">
                        <span class="flex-shrink-0 w-6 h-6 rounded-full bg-indigo-100 text-indigo-700 text-xs font-semibold flex items-center justify-center">4</span>
                        <span>{{ __('Click "Place order & send tickets" — the customer will receive them by email.') }}</span>
                    </li>
                    <li class="flex gap-3">
                        <span class="flex-shrink-0 w-6 h-6 rounded-full bg-indigo-100 text-indigo-700 text-xs font-semibold flex items-center justify-center">5</span>
