...
<li>
    <a
        ...
        @class([
            ...
            'bg-[color:var(--ds-sidebar-active-bg)] text-[color:var(--ds-sidebar-active-text)]' => $active,
    ...
    >
        @if ($icon && isset($icons[$icon]))
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" @class(['flex-shrink-0', $active ? 'text-[color:var(--ds-accent-text)]' : 'text-[color:var(--ds-text-muted)] group-hover:text-[color:var(--ds-text)]'])" aria-hidden="true">
                <path d="{{ $icons[$icon] }}"></path>
            </svg>
        @endif
        <span class="flex-1 truncate">{{ $slot }}</span>
        @if ($active)
            <span class="w-1.5 h-1.5 rounded-full bg-[color:var(--ds-accent)]"></span>
        @endif
    </a>
</li>
